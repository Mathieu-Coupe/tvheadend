...
htsmsg_xml_parse_attrib(xmlparser_t *xp, htsmsg_t *msg, char *src,
	...
	if (xmlns_scope_list != NULL && attriblen > 6
			&& !memcmp(attribname, "xmlns:", 6)) {
...
htsmsg_xml_parse_tag(xmlparser_t *xp, htsmsg_t *parent, char *src) {
		...
		if (tagname[i] == ':') {
			...
			LIST_FOREACH(ns, &xp->xp_namespaces, xmlns_global_link) {
				if (ns->xmlns_prefix_len == i
						&& !memcmp(ns->xmlns_prefix, tagname,
								ns->xmlns_prefix_len)) {
	...
	done: while ((ns = LIST_FIRST(&nslist)) != NULL)
		xmlns_destroy(ns);
	return src;
}
 ...
 */
static char *
decode_label_reference(xmlparser_t *xp, struct cdata_content_queue *ccq,
		char *src) {
	char *s = src;
	int l;
	char *label;
 ...
 */
static char *
htsmsg_xml_parse_cd0(xmlparser_t *xp, struct cdata_content_queue *ccq,
		htsmsg_t *tags, htsmsg_t *pis, char *src, int raw) {
