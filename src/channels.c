...
static void channel_tag_mapping_destroy(channel_tag_mapping_t *ctm, int flags);
 ...
 *
 */
static void channel_list_changed(void) {
 ...
 *
 */
static int channelcmp(const channel_t *a, const channel_t *b) {
 ...
 *
 */
static int chidcmp(const channel_t *a, const channel_t *b) {
 ...
 *
 */
static void channel_set_name(channel_t *ch, const char *name) {
	channel_t *x;
	const char *n2;
	int l, i;
	char *cp, c;
 ...
 *
 */
static void channel_load_one(htsmsg_t *c, int id) {
	channel_t *ch;
	const char *s;
	...
	if ((s = htsmsg_get_str(c, "xmltv-channel")) != NULL && (ch->ch_xc =
			xmltv_channel_find(s, 0)) != NULL) {
...
void channel_delete(channel_t *ch) {
	...
	tvhlog(LOG_NOTICE, "channels", "Channel \"%s\" deleted", ch->ch_name);
...
void channel_set_epg_postpre_time(channel_t *ch, int pre, int mins) {
	...
	tvhlog(LOG_NOTICE, "channels",
			"Channel \"%s\" epg %s-time set to %d minutes", ch->ch_name,
			pre ? "pre" : "post", mins);
 ...
 *
 */
void channel_set_xmltv_source(channel_t *ch, xmltv_channel_t *xc) {
	lock_assert(&global_lock);
	...
	for (ctm = LIST_FIRST(&ch->ch_ctms); ctm != NULL; ctm = n) {
		...
		if (ctm->ctm_mark) {
			...
			channel_tag_mapping_destroy(ctm,
					CTM_DESTROY_UPDATE_TAG | CTM_DESTROY_UPDATE_CHANNEL);
		}
	}
 ...
 *
 */
static void channel_tag_destroy(channel_tag_t *ct) {
	channel_tag_mapping_t *ctm;
	channel_t *ch;
 ...
 *
 */
static int channel_tag_record_delete(void *opaque, const char *id) {
	channel_tag_t *ct;
 ...
 *
 */
static const dtable_class_t channel_tags_dtc = { .dtc_record_get =
		channel_tag_record_get,
		.dtc_record_get_all = channel_tag_record_get_all, .dtc_record_create =
				channel_tag_record_create, .dtc_record_update =
				channel_tag_record_update, .dtc_record_delete =
				channel_tag_record_delete, .dtc_read_access = ACCESS_ADMIN,
		.dtc_write_access = ACCESS_ADMIN, .dtc_mutex = &global_lock, };
