 ...
 *
 */
static void htsmsg_json_encode_string(const char *str, htsbuf_queue_t *hq) {
	const char *s = str;
 ...
 */
static void htsmsg_json_write(htsmsg_t *msg, htsbuf_queue_t *hq, int isarray,
...
static const char *htsmsg_json_parse_value(const char *s, htsmsg_t *parent,
		char *name);
 ...
 */
static double _strntod(const char *s, char decimal_point_char, char **ep) {
	...
	for (c = s; *c != '\0' && ((*c > 0 && *c < 33) || /* skip whitespace */
