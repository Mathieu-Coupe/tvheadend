 ...
 *
 */
static void dvrdb_changed(void) {
 ...
 *
 */
static void dvrconfig_changed(void) {
	htsmsg_t *m = htsmsg_create_map();
	htsmsg_add_u32(m, "reload", 1);
 ...
 *
 */
static void dvr_make_title(char *output, size_t outlen, dvr_entry_t *de) {
	struct tm tm;
	char buf[40];
	int i;
	...
	snprintf(output + strlen(output), outlen - strlen(output), "%s",
			de->de_title);
	...
	if (cfg->dvr_flags & DVR_EPISODE_IN_TITLE) {
		...
		if (de->de_episode.ee_season && de->de_episode.ee_episode)
			snprintf(output + strlen(output), outlen - strlen(output),
					".S%02dE%02d", de->de_episode.ee_season,
					de->de_episode.ee_episode);

		else if (de->de_episode.ee_episode)
			snprintf(output + strlen(output), outlen - strlen(output), ".E%02d",
					de->de_episode.ee_episode);
	}

	if (cfg->dvr_flags & DVR_CLEAN_TITLE) {
		for (i = 0; i < strlen(output); i++) {
			if (output[i] < 32 || output[i] > 122 || output[i] == 34
					|| output[i] == 39 || output[i] == 92 || output[i] == 58)
				output[i] = '_';
		}
	}
}
 ...
 *
 */
static void dvr_entry_link(dvr_entry_t *de) {
	time_t now, preamble;
	char buf[100];
	dvr_config_t *cfg = dvr_config_find_by_name_default(de->de_config_name);
 ...
 */
dvr_entry_t *
dvr_entry_create(const char *config_name, channel_t *ch, time_t start,
		time_t stop, const char *title, const char *description,
		const char *creator, dvr_autorec_entry_t *dae, epg_episode_t *ee,
	...
	tvhlog(LOG_INFO, "dvr", "\"%s\" on \"%s\" starting at %s, "
			"scheduled for recording by \"%s\"", de->de_title,
			de->de_channel->ch_name, tbuf, creator);
 ...
 */
dvr_entry_t *
dvr_entry_create_by_event(const char *config_name, event_t *e,
	...
	return dvr_entry_create(config_name, e->e_channel, e->e_start, e->e_stop,
			e->e_title, desc, creator, dae, &e->e_episode, e->e_content_type,
			pri);
}
 ...
 *
 */
static void dvr_entry_remove(dvr_entry_t *de) {
 ...
 *
 */
static void dvr_db_load_one(htsmsg_t *c, int id) {
	dvr_entry_t *de;
 ...
 */
dvr_entry_t *
dvr_entry_update(dvr_entry_t *de, const char* de_title, int de_start,
		int de_stop) {
	...
	tvhlog(LOG_INFO, "dvr", "\"%s\" on \"%s\": Updated Timer", de->de_title,
			de->de_channel->ch_name);
 ...
 *
 */
static void dvr_stop_recording(dvr_entry_t *de, int stopcode) {
	dvr_config_t *cfg = dvr_config_find_by_name_default(de->de_config_name);
	...
	tvhlog(LOG_INFO, "dvr", "\"%s\" on \"%s\": "
			"End of program: %s", de->de_title, de->de_channel->ch_name,
...
static void dvr_timer_start_recording(void *aux) {
	...
	tvhlog(LOG_INFO, "dvr", "\"%s\" on \"%s\" recorder starting", de->de_title,
			de->de_channel->ch_name);
...
dvr_entry_find_by_event(event_t *e) {
	...
	LIST_FOREACH(de, &e->e_channel->ch_dvrs, de_channel_link)
		if (de->de_start == e->e_start && de->de_stop == e->e_stop)
			return de;
	return NULL;
}
...
dvr_entry_find_by_event_fuzzy(event_t *e) {
	...
	LIST_FOREACH(de, &e->e_channel->ch_dvrs, de_channel_link)
		if ((abs(de->de_start - e->e_start) < 600)
				&& (abs(de->de_stop - e->e_stop) < 600)) {
			return de;
		}
	return NULL;
}
 ...
 */
static void dvr_entry_purge(dvr_entry_t *de) {
...
void dvr_init(void) {
	...
	if (l != NULL) {
		HTSMSG_FOREACH(f, l) {
			...
			tvh_str_set(&cfg->dvr_file_postfix,
					htsmsg_get_str(m, "file-postfix"));
 ...
 *
 */
static void dvr_save(dvr_config_t *cfg) {
	htsmsg_t *m = htsmsg_create_map();
	...
	htsmsg_add_u32(m, "episode-in-title",
			!!(cfg->dvr_flags & DVR_EPISODE_IN_TITLE));
...
void dvr_format_set(dvr_config_t *cfg, const char *format) {
	...
	tvhlog(LOG_DEBUG, "dvr", "Saving format: \"%s\" was : \"%s\"", format,
			cfg->dvr_format);
...
void dvr_file_postfix_set(dvr_config_t *cfg, const char *file_postfix) {
	if (cfg->dvr_file_postfix != NULL
			&& !strcmp(cfg->dvr_file_postfix, file_postfix))
		return;

	tvhlog(LOG_DEBUG, "dvr", "Saving file_postfix: \"%s\" was : \"%s\"",
			file_postfix, cfg->dvr_file_postfix);
 ...
 *
 */
static void dvr_query_add_entry(dvr_query_result_t *dqr, dvr_entry_t *de) {
 ...
 * Sorting functions
 */
static int dvr_sort_start_descending(const void *A, const void *B) {
 ...
 *
 */
static struct strtab priotab[] = { { "important", DVR_PRIO_IMPORTANT }, {
		"high", DVR_PRIO_HIGH }, { "normal", DVR_PRIO_NORMAL }, { "low",
		DVR_PRIO_LOW }, { "unimportant", DVR_PRIO_UNIMPORTANT }, };
...
void dvr_entry_delete(dvr_entry_t *de) {
	if (de->de_filename != NULL) {
		if (unlink(de->de_filename) && errno != ENOENT)
			tvhlog(LOG_WARNING, "dvr",
					"Unable to remove file '%s' from disk -- %s",
