 ...
 *
 */
int tcp_connect(const char *hostname, int port, char *errbuf, size_t errbufsize,
		int timeout) {
	const char *errtxt;
	...
	while ((res = gethostbyname_r(hostname, &hostbuf, tmphstbuf, hstbuflen, &hp,
			&herr)) == ERANGE) {
		hstbuflen *= 2;
 ...
 *
 */
static int tcp_fill_htsbuf_from_fd(int fd, htsbuf_queue_t *hq) {
	...
	while (1) {
		...
		for (i = 0; i < r; i++) {
			...
			if (ev[i].events & EPOLLIN) {
				...
				tsl->fd = accept(ts->serverfd, (struct sockaddr *) &tsl->peer,
						&slen);
				if (tsl->fd == -1) {
					perror("accept");
					free(tsl);
				...
				slen = sizeof(struct sockaddr_in);
				if (getsockname(tsl->fd, (struct sockaddr *) &tsl->self,
						&slen)) {
					close(tsl->fd);
					free(tsl);
					continue;
