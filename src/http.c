...
static const char *cachedays[7] = { "Sun", "Mon", "Tue", "Wed", "Thu", "Fri",
		"Sat" };

static const char *cachemonths[12] = { "Jan", "Feb", "Mar", "Apr", "May", "Jun",
...
void http_send_header(http_connection_t *hc, int rc, const char *content,
		int64_t contentlen, const char *encoding, const char *location,
		int maxage, const char *range, const char *disposition) {
	struct tm tm0, *tm;
	htsbuf_queue_t hdrs;
	time_t t;
	...
	htsbuf_qprintf(&hdrs, "%s %d %s\r\n",
			val2str(hc->hc_version, HTTP_versiontab), rc, http_rc2str(rc));
	...
	if (maxage == 0) {
		...
		htsbuf_qprintf(&hdrs,
				...
				cachemonths[tm->tm_mon], tm->tm_mday, tm->tm_hour, tm->tm_min,
				tm->tm_sec);

		t += maxage;

		tm = gmtime_r(&t, &tm0);
		htsbuf_qprintf(&hdrs, "Expires: %s, %02d %s %d %02d:%02d:%02d GMT\r\n",
				cachedays[tm->tm_wday], tm->tm_year + 1900,
				cachemonths[tm->tm_mon], tm->tm_mday, tm->tm_hour, tm->tm_min,
				tm->tm_sec);
 ...
 * Transmit a HTTP reply
 */
static void http_send_reply(http_connection_t *hc, int rc, const char *content,
		const char *encoding, const char *location, int maxage) {
	http_send_header(hc, rc, content, hc->hc_reply.hq_size, encoding, location,
			maxage, 0, NULL);
...
void http_error(http_connection_t *hc, int error) {
	...
	tvhlog(LOG_ERR, "HTTP", "%s: %s -- %d", inet_ntoa(hc->hc_peer->sin_addr),
			hc->hc_url, error);
...
void http_output_html(http_connection_t *hc) {
	return http_send_reply(hc, HTTP_STATUS_OK, "text/html; charset=UTF-8", NULL,
			NULL, 0);
}
...
int http_access_verify(http_connection_t *hc, int mask) {
	...
	if (!access_ticket_verify(ticket_id, hc->hc_url)) {
		tvhlog(LOG_INFO, "HTTP", "%s: using ticket %s for %s",
				inet_ntoa(hc->hc_peer->sin_addr), ticket_id, hc->hc_url);
		return 0;
	}
...
static int process_request(http_connection_t *hc, htsbuf_queue_t *spill) {
	...
	if (hc->hc_username != NULL) {
		...
		/* Not threadsafe ? */
		snprintf(hc->hc_representative, 30, "%s",
 ...
 *
 */
static void http_serve_requests(http_connection_t *hc, htsbuf_queue_t *spill) {
	char cmdline[1024];
	char hdrline[1024];
	char *argv[3], *c;
	int n;
 ...
 *
 */
static void http_serve(int fd, void *opaque, struct sockaddr_in *peer,
