 ...
 *
 */
void streaming_target_init(streaming_target_t *st, st_callback_t *cb,
 ...
 *
 */
static void streaming_queue_deliver(void *opauqe, streaming_message_t *sm) {
 ...
 *
 */
static void streaming_queue_deliver2(void *opauqe, streaming_message_t *sm) {
	streaming_queue_t *sq = opauqe;
	...
	if (sq->sq_tsbuf) {
		streaming_message_t *sm2 = streaming_msg_create_data(SMT_MPEGTS,
				sq->sq_tsbuf);
		sq->sq_tsbuf = NULL;
		pthread_mutex_lock(&sq->sq_mutex);
...
void streaming_queue_init(streaming_queue_t *sq, int reject_filter) {
	if ((reject_filter & SMT_TO_MASK(SMT_PACKET)) == 0) {
		...
		streaming_target_init(&sq->sq_st, streaming_queue_deliver2, sq,
				reject_filter);
		sq->sq_st.st_clone = 0;
	}
 ...
 *
 */
int streaming_pad_probe_type(streaming_pad_t *sp, streaming_message_type_t smt) {
