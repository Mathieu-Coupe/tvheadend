 ...
 *
 */
void serviceprobe_enqueue(service_t *t) {
 ...
 *
 */
void serviceprobe_delete(service_t *t) {
	if (!t->s_sp_onqueue)
		return;
...
serviceprobe_thread(void *aux) {
	...
	while (1) {
		...
		tvhlog(LOG_INFO, "serviceprobe", "%20s: checking...", t->s_svcname);
		...
		if (t->s_status != SERVICE_ZOMBIE) {
			...
			if (err != NULL) {
				...
				tvhlog(LOG_INFO, "serviceprobe",
						"%20s: mapped to channel \"%s\"", t->s_svcname,
						t->s_svcname);

				if (service_is_tv(t)) {
					channel_tag_map(ch,
							channel_tag_find_by_name("TV channels", 1), 1);
				...
				if (t->s_provider != NULL) {
					channel_tag_map(ch,
							channel_tag_find_by_name(t->s_provider, 1), 1);
					tvhlog(LOG_INFO, "serviceprobe", "%20s: joined tag \"%s\"",
